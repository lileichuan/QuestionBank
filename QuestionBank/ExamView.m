...
@interface ExamView()<QuestionBrowserDelegate,QuestionProtocol >{
    UIView *guideView;
    TopView *topView;
...
-(void)addQuestionResultView{
    ...
    if (!questionResultView) {
        ...
        [self addSubview:questionResultView];
        [questionResultView release];
    }
    self.viewTag = 2;
}
...
- (QuestionCell *) gridView:(UIGridView *)grid cellForRowAt:(int)rowIndex AndColumnAt:(int)columnIndex
{
	...
	if (cell == nil) {
        float width = CGRectGetWidth(self.bounds) / 6;
        float heigth = CGRectGetWidth(self.bounds) / 6;
