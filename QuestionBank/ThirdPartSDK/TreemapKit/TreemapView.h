...
@interface TreemapView : UIView <TreemapViewCellDelegate> {
   IBOutlet id <TreemapViewDataSource> dataSource;
   IBOutlet id <TreemapViewDelegate> delegate;

    BOOL initialized;
}

@property (nonatomic, retain) IBOutlet id <TreemapViewDataSource> dataSource;
@property (nonatomic, assign) IBOutlet id <TreemapViewDelegate> delegate;
